...
def decode_image(blob_data):
    try:
        ...
        return Image.open(io.BytesIO(blob_data))  # Reabre para operação normal após verificação
...
def detect_objects_in_image(image):
    ...
    logger.info(f"Detecção concluída: {people_count} pessoas e {car_count} veículos detectados.")
...
def get_circuit_limits(connection, circuito_id):
    logger.info(f"Buscando limites de pessoas e veículos para o circuito {circuito_id}.")
    try:
        with connection.cursor() as cursor:
            ...
            if result:
                logger.info(f"Limites obtidos: {result[0]} pessoas, {result[1]} veículos.")
...
def is_within_allowed_time(start_time, end_time):
    ...
    logger.debug(f"Horário atual: {now}, Horário permitido: {start_time.time()} - {end_time.time()}")
...
def process_images_from_db(connection, batch_size=10):
    logger.info(f"Buscando imagens não classificadas no banco de dados (limite: {batch_size}).")
    try:
        with connection.cursor() as cursor:
            ...
            for image_row in images:
                ...
                logger.debug(f"Tamanho do blob para imagem ID {img_id}: {len(blob_data)} bytes.")
                ...
                circuito_pessoas, circuito_veiculos, horario_inicio, horario_fim = get_circuit_limits(connection, circuito_id)
                
                if circuito_pessoas is None or circuito_veiculos is None:
                    logger.error(f"Falha ao buscar limites para o circuito {circuito_id}, pulando.")
                    continue
                ...
                if not is_within_allowed_time(horario_inicio, horario_fim):
                    ...
                    if people_count > 0:
                        logger.warning(f"Imagem ID {img_id}: Alerta! Detecção fora do horário permitido.")
                        tipo_classificado = 1  # Alerta
                    else:
                        logger.info(f"Imagem ID {img_id}: Nenhuma pessoa detectada fora do horário permitido.")
                    ...
                    if people_count > circuito_pessoas or car_count > circuito_veiculos:
                        logger.warning(f"Imagem ID {img_id}: Alerta! Quantidade acima do permitido.")
                        tipo_classificado = 1  # Alerta
                    else:
...
def main():
    ...
    while True:
        ...
        try:
            ...
            has_processed_images = process_images_from_db(connection, batch_size=batch_size)
            
            if not has_processed_images:
                logger.info(f"Aguardando {interval} segundos antes de verificar novamente.")
                time.sleep(interval)
